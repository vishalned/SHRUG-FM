...
https://github.com/spaceml-org/ml4floods/blob/main/ml4floods/models/dataset_setup.py
With some modifications to make it work for us
"""
...
from ml4floods.preprocess.tiling import (
    Dict_to_WindowSlices,
    WindowSlices,
    WindowSlices_to_Dict,
)
...
def filenames_train_test_split(
    bucket_name: Optional[str], train_test_split_file: str
...
def process_filename_train_test(
    train_test_split_file: Optional[
        str
    ] = "gs://ml4cc_data_lake/2_PROD/2_Mart/worldfloods_v1_0/train_test_split.json",
    input_folder: str = "S2",
    target_folder: str = "gt",
    bucket_id: Optional[str] = None,
    path_to_splits: Optional[str] = None,
    download: Optional[Dict[str, bool]] = None,
) -> Dict[str, Dict[str, List[str]]]:
    """
    ...
    if train_test_split_file:
        ...
        assert (path_to_splits is not None) and os.path.exists(
            path_to_splits
    ...
    for isplit in ["train", "test", "val"]:
        for foldername in [input_folder, target_folder]:
            ...
            if len(filenames_train_test[isplit][foldername]) == 0:
                ...
                assert (
                    len(filenames_train_test[isplit][foldername]) > 0
        ...
        ), f"""Different number of files in {input_folder} and {target_folder} for split {isplit}:\n
            {len(filenames_train_test[isplit][input_folder])} {len(filenames_train_test[isplit][target_folder])}"""
        ...
        for idx, filename in enumerate(filenames_train_test[isplit][input_folder]):
            # subst_filename = filename.split("/")[-3:]
            # subst_filename = os.path.join(
            #     path_to_splits, isplit, input_folder, f"{filename}.tif"
            # )
            # this is a bit of a post-hoc hack
            filename = os.path.join(path_to_splits, filename)
            # filenames_train_test[isplit][input_folder][idx] = filename
            ...
            filename_target = os.path.join(path_to_splits, filename_target)
            # filenames_train_test[isplit][target_folder][idx] = filename_target

            # subst_filename_target = filename_target.split("/")[-3:]
            # subst_filename_target = os.path.join(
            #     path_to_splits, isplit, target_folder, f"{filename_target}.tif"
            # )
...
def get_dataset(
    data_config: Dynaconf, data_path: str, bootstrap_seed: int | None = None
    ...
    Function to set up dataloaders for model training
        Args:
            ...
            bootstrap_seed: set to an integer to bootstrap the training set
        Returns:
            pl.LightningDataModule
    """
    ...
    path_to_splits = os.path.join(data_path, "floods/worldfloods_v2")
    ...
    if data_config.get("filter_windows_apply", False):
        filter_windows_config = filter_windows_fun(
            data_config.filter_windows_version,
            data_config.train_test_split_file,
            windows_file=data_config.get("windows_file"),
            threshold_clouds=data_config.filter_windows_threshold,
    ...
    datamodule = WorldFloodsDataModule(
        ...
        window_size=[data_config.img_size, data_config.img_size],
    ...
    logger = get_logger_initialized()
    logger.info(f"train: {datamodule.train_dataset.__len__()} tiles")
    logger.info(f"val: {datamodule.val_dataset.__len__()} tiles")
    logger.info(f"test: {datamodule.test_dataset.__len__()} tiles")

    return datamodule


def load_windows(filename: str, split: str) -> List[WindowSlices]:
    with open(filename, "r") as fh:
        split_slices = json.load(fh)
    if split in split_slices:
        return [Dict_to_WindowSlices(dictio) for dictio in split_slices[split]]
    else:
        return None


def save_windows(list_windows: List[WindowSlices], filename: str, split: str) -> None:
...
def filter_windows_fun(
    ...
    local_destination_dir: Optional[str] = None,
    windows_file: Optional[str] = None,
    ...
    """
    if windows_file is None and local_destination_dir is not None:
        if train_test_split_file:
    ...
    def filter_windows(tiledDataset: WorldFloodsDatasetTiled, split: str) -> List[WindowSlices]:
        selected_windows = None
        windows_file = '/data/users/maria/shrug-fm/window_file.json'
        if windows_file and os.path.exists(windows_file):
            selected_windows = load_windows(windows_file, split)
        ...
        if selected_windows is None:
            ...
            if windows_file:
                # usmile fix 
                windows_file = '/data/users/maria/shrug-fm/window_file.json'
                print("Saving windows to", windows_file)
                save_windows(selected_windows, windows_file, split)
...
def get_transformations(data_config) -> Tuple[Callable, Callable]:
    ...
    train_transform = [
        ...
        A.RandomRotate90(always_apply=True, p=0.5),
    ...
    channel_mean = None
    if data_config.normalize_train:
        ...
        train_transform.append(
            A.Normalize(
                mean=channel_mean[0, 0].tolist(), std=channel_std[0, 0].tolist(), max_pixel_value=1
            )
        )
    ...
    train_transform = A.Compose(train_transform, is_check_shapes=False)
    ...
    if data_config.normalize_test:
        ...
        test_transform = A.Compose(test_transform, is_check_shapes=False)
    else:
