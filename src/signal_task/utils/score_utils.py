...
def add_negative_pred_normalization(
    metrics: pd.DataFrame, variable: "Variance", full_image_area: int = 512 * 512
) -> pd.DataFrame:
    """ """

    # Compute additional metric (Uncertainty over negative prediction)
    variable_pred = metrics[f"{variable}_pred_normalized"] * metrics["Pred_Area"]
    variable_total = metrics[f"{variable}"] * full_image_area
    neg_pred_area = full_image_area - metrics["Pred_Area"]
    metrics[f"{variable}_neg_pred_normalized"] = (variable_total - variable_pred) / neg_pred_area

    return metrics



def metric_scaled(metric_map: np.ndarray, mask: np.ndarray, probability_threshold: float = None) -> np.ndarray:
    """
    Compute the mean of the metric map over the mask, with optional probability-based masking.

    Args:
        metric_map (np.ndarray): The metric map to compute the mean over.
        mask (np.ndarray): The mask to compute the mean over. Can be a binary mask or probability array.
        probability_threshold (float, optional): If provided, creates a binary mask by thresholding
            the input mask at this probability value. If None, uses the mask as-is.

    Returns:
        np.ndarray: The mean of the metric map over the (possibly thresholded) mask.
    """

    if metric_map.shape[1] == 2:
        metric_map = metric_map[:, 1]

    if probability_threshold is not None:
        binary_mask = (mask >= probability_threshold).astype(np.float32)
    else:
        binary_mask = mask

    metric_sum_over_mask = (metric_map * binary_mask).sum(axis=(1, 2))
    pixel_count = binary_mask.sum(axis=(1, 2))
    mean_metric_over_mask = np.divide(
        metric_sum_over_mask,
        pixel_count,
        out=np.zeros_like(metric_sum_over_mask),
        where=pixel_count > 0,
    )
    return mean_metric_over_mask


def add_metrics_over_support(metrics: pd.DataFrame) -> pd.DataFrame:
    """ """
    image_size = 512 * 512
    scaling_factors = metrics["Prob_Area"] / image_size

    for variable in ["Probability", "Entropy", "Variance", "Mutual Information"]:
        metrics[f"{variable}_support_scaled"] = metrics[variable] * scaling_factors
...
def weighted_rmse(pred, target):
    ...
    return np.sqrt(
        1 / num_lat * 1 / num_long * np.sum(np.dot(weight.T, (pred[0] - target[0]) ** 2))
    )
                ...
                for n in range(width):
                    if m + 1 < height:
                        if mask[i][j][0][m + 1][n] and mask[i][j][0][m][n]:
                            pred_diff_h = abs(
                                prediction[i][j][0][m + 1][n] - prediction[i][j][0][m][n]
                            )
                    ...
                    if n + 1 < width:
                        if mask[i][j][0][m][n + 1] and mask[i][j][0][m][n]:
                            pred_diff_w = abs(
                                prediction[i][j][0][m][n + 1] - prediction[i][j][0][m][n]
                            )
...
def top_quantiles_error_torch(
    ...
    qcut = 1
    n, c, h, w = pred.size()
    qtile = 1.0 - 2 * torch.logspace(-qlim, -qcut, steps=qs, device=pred.device, dtype=target.dtype)
    P_tar = torch.quantile(target.view(n, c, h * w), q=qtile, dim=-1)
    qtile = 1.0 - 2 * torch.logspace(-qlim, -qcut, steps=qs, device=pred.device, dtype=pred.dtype)
    P_pred = torch.quantile(pred.view(n, c, h * w), q=qtile, dim=-1)
...
def lower_quantiles_error_torch(
    ...
    qtile = 2 * torch.logspace(-qlim, -qcut, steps=qs, device=pred.device, dtype=target.dtype)

    # Compute quantiles for the target and predictions
    P_tar = torch.quantile(target.view(n, c, h * w), q=qtile, dim=-1)
    qtile = 2 * torch.logspace(-qlim, -qcut, steps=qs, device=pred.device, dtype=pred.dtype)
    P_pred = torch.quantile(pred.view(n, c, h * w), q=qtile, dim=-1)
